...
use a5_rs::core::cell::{lonlat_to_cell, cell_to_boundary, a5cell_contains_point, CellToBoundaryOptions};
...
fn test_antimeridian_cell_longitude_span() {
    ...
    for cell_id_hex in &antimeridian_cells {
        for &segment in &segments {
            ...
            let cell_id_u64 = cell_id.to_string().parse::<u64>().expect("Failed to convert to u64");
            
            let options = CellToBoundaryOptions {
                closed_ring: true,
                segments: Some(segment),
            };
            let boundary = cell_to_boundary(cell_id_u64, Some(options)).expect("Failed to get boundary");
...
fn test_cell_boundary_contains_original_point() {
    ...
    let test_points: Vec<LonLat> = populated_places.features.iter()
        .map(|feature| {
    ...
    println!("Testing with {} points from GeoJSON file", test_points.len());
    ...
    println!("Skipping resolution {} as lonLatToCell is not implemented for this resolution yet", MAX_RESOLUTION);
    
    // Test each point from GeoJSON
    for (point_index, test_lonlat) in test_points.iter().enumerate() {
        let feature_name = populated_places.features[point_index].properties.name
            .clone()
            .unwrap_or_else(|| format!("Unnamed {}", point_index));
        let point_key = format!("Point {} - {} ({}, {})", point_index, feature_name, test_lonlat.longitude(), test_lonlat.latitude());
        ...
        for resolution in 1..MAX_RESOLUTION {
                    ...
                    match cell_to_boundary(cell_id, None) {
                        Ok(_boundary) => {
                            ...
                            match deserialize(cell_id) {
                                Ok(cell) => {
                                    match a5cell_contains_point(&cell, *test_lonlat) {
                                        Ok(contains_result) => {
                                        ...
                                        Err(e) => {
                                            resolution_failures.push(format!("Error checking containment: {}", e));
                                        }
                                    }
                                }
                                Err(e) => {
                                    resolution_failures.push(format!("Error deserializing cell: {}", e));
                                }
                            }
                        }
                        Err(e) => {
            ...
            if !resolution_failures.is_empty() {
                failures.entry(point_key.clone()).or_default().insert(resolution, resolution_failures);
            }
        }
    }
