...
fn test_dodecahedron_forward_projections() {
    ...
    let origin_id: OriginId = test_data["static"]["ORIGIN_ID"]
        .as_u64()
        .expect("Origin ID should be a number") as u8;
    let mut dodecahedron =
        DodecahedronProjection::new().expect("Failed to create DodecahedronProjection");

    let forward_tests = test_data["forward"]
        .as_array()
        .expect("Forward tests should be an array");
    ...
    for test_case in forward_tests {
        ...
        let result = dodecahedron
            .forward(spherical, origin_id)
...
fn test_dodecahedron_inverse_projections() {
    ...
    let inverse_tests = test_data["inverse"]
        .as_array()
        .expect("Inverse tests should be an array");
    ...
    for test_case in inverse_tests {
        ...
        assert_relative_eq!(
            ...
            expected[0]
                .as_f64()
                .expect("Expected theta should be a number"),
            epsilon = TOLERANCE
        );
        assert_relative_eq!(
            result.phi().get(),
            expected[1]
                .as_f64()
                .expect("Expected phi should be a number"),
            epsilon = TOLERANCE
        );
    }
}
...
fn test_dodecahedron_forward_round_trip() {
    ...
    for test_case in forward_tests.iter().take(20) {
...
fn test_dodecahedron_inverse_round_trip() {
    ...
    for test_case in inverse_tests.iter().take(20) {
        // Test first 20 for performance
        let input = test_case["input"]
            .as_array()
            .expect("Input should be an array");
        ...
        assert_relative_eq!(result.x(), face.x(), epsilon = TOLERANCE);
        assert_relative_eq!(result.y(), face.y(), epsilon = TOLERANCE);
    }
}
...
fn test_dodecahedron_error_handling() {
    ...
    let spherical = Spherical::new(Radians::new_unchecked(0.0), Radians::new_unchecked(0.0));
